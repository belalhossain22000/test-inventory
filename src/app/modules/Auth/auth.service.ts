...
// user login
const loginUser = async (payload: { name: string; password: string }) => {
  ...
  if (!userData?.name) {
    throw new ApiError(
      ...
      "User not found! with this name " + payload.name
    );
  }
...
const forgotPassword = async (payload: { name: string }) => {
  ...
  const resetPassToken = jwtHelpers.generateToken(
    { email: userData.name, role: userData.role, id: userData.id },
